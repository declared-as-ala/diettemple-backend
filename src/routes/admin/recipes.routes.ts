...
function normalizeIngredientsInput(input: any): Array<{ name: string; normalizedName: string; quantity?: number; unit?: string }> {
  ...
  return input
    .map((ing: any) => {
      if (typeof ing === 'string') {
        const n = ing.trim();
        if (!n) return null;
        return { name: n, normalizedName: normalizeIngredientName(n) };
      }
      const name = String(ing?.name || '').trim();
      if (!name) return null;
      const quantity = ing?.quantity !== undefined && ing?.quantity !== '' ? Number(ing.quantity) : undefined;
      const unit = ing?.unit ? String(ing.unit).trim() : undefined;
      return {
        name,
    ...
    .filter((v): v is { name: string; normalizedName: string; quantity?: number; unit?: string } => !!v);
}
...
router.post(
  ...
  body('preparationTimeMinutes').optional({ nullable: true }).isNumeric(),
  body('mealPrepDays').optional().isArray(),
  body('isBatchCookingFriendly').optional().isBoolean(),
    ...
    try {
      ...
      const doc = await Recipe.create({
        ...
        preparationTimeMinutes:
          req.body.preparationTimeMinutes !== undefined && req.body.preparationTimeMinutes !== ''
            ? Number(req.body.preparationTimeMinutes)
            : null,
        preparationTimeLabel:
          req.body.preparationTimeMinutes !== undefined && req.body.preparationTimeMinutes !== ''
            ? `${Number(req.body.preparationTimeMinutes)} min`
            : undefined,
        mealPrepDays: Array.isArray(req.body.mealPrepDays) ? req.body.mealPrepDays.map(Number).filter((n: number) => [2, 3, 4].includes(n)) : [],
        isBatchCookingFriendly: !!req.body.isBatchCookingFriendly,
        servings: req.body.servings !== undefined && req.body.servings !== '' ? Number(req.body.servings) : undefined,
        storageInstructions: req.body.storageInstructions || undefined,
        ingredients: normalizeIngredientsInput(req.body.ingredients),
      });
        ...
        req.params.id,
        {
          $set: {
            ...
            ...(req.body.preparationTimeMinutes !== undefined && {
              preparationTimeMinutes:
                req.body.preparationTimeMinutes === '' || req.body.preparationTimeMinutes == null
                  ? null
                  : Number(req.body.preparationTimeMinutes),
              preparationTimeLabel:
                req.body.preparationTimeMinutes === '' || req.body.preparationTimeMinutes == null
            ...
            ...(req.body.mealPrepDays !== undefined && {
              mealPrepDays: Array.isArray(req.body.mealPrepDays)
                ? req.body.mealPrepDays.map(Number).filter((n: number) => [2, 3, 4].includes(n))
                : [],
            }),
            ...(req.body.isBatchCookingFriendly !== undefined && { isBatchCookingFriendly: !!req.body.isBatchCookingFriendly }),
            ...(req.body.servings !== undefined && {
              servings: req.body.servings === '' || req.body.servings == null ? undefined : Number(req.body.servings),
            }),
            ...(req.body.storageInstructions !== undefined && { storageInstructions: req.body.storageInstructions || undefined }),
            ...(req.body.ingredients !== undefined && { ingredients: normalizeIngredientsInput(req.body.ingredients) }),
          },
        },
        { new: true }
      );
