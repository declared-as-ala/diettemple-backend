...
import fs from 'fs';
import { dietTempleLogoCid, dietTempleLogoPath } from '../utils/brandAssets';
...
test('renders full address without missing legacy fields and escapes customer and product content', () => {
  ...
  expect(html).toContain(`src="cid:${dietTempleLogoCid}"`);
...
test('sends customer HTML and plain-text confirmation through configured transport', async () => {
  ...
  expect(transport.sendMail.mock.calls[0][0].attachments).toEqual([
    expect.objectContaining({ path: dietTempleLogoPath, cid: dietTempleLogoCid, contentDisposition: 'inline' }),
  ]);
  expect(fs.readFileSync(dietTempleLogoPath).subarray(1, 4).toString()).toBe('PNG');
});
