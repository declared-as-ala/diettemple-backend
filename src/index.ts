...
mongoose
  ...
  .then(async () => {
    ...
    // Init MinIO buckets (non-blocking — server starts even if MinIO is slow)
    initMinioBuckets().catch((err) =>
      console.error('[minio] Bucket init failed (uploads will fail until resolved):', err?.message)
    );
