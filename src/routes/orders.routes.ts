...
router.post(
  ...
  [
    ...
    body('deliveryAddress.fullName').isString().bail().trim().notEmpty().withMessage('Nom et prénom requis'),
    body('deliveryAddress.street').isString().bail().trim().notEmpty().withMessage('Adresse complète requise'),
    body('deliveryAddress.city').optional().isString().trim(),
    body('deliveryAddress.delegation').optional().isString().trim(),
    body('deliveryAddress.phone').isString().bail().trim().customSanitizer(value => value.replace(/[\s().-]/g, '')).matches(/^(\+216|00216)?[0-9]{8}$/).withMessage('Téléphone invalide'),
    body('deliveryAddress.email').isString().bail().trim().isEmail().withMessage('Email invalide'),
