import { dietTempleLogoCid } from '../utils/brandAssets';
...
export function orderEmailHtml(order: IOrder, admin = false): string {
  ...
  <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="width:100%;max-width:600px;background:#fcfcf7;border-radius:24px;overflow:hidden">
    <tr><td style="padding:32px 26px;background:#182112;border-bottom:4px solid #dfc477"><table role="presentation" cellspacing="0" cellpadding="0" style="margin-bottom:30px"><tr><td style="padding-right:18px"><img src="cid:${dietTempleLogoCid}" width="70" height="70" alt="Logo DietTemple" style="display:block;border:0;width:70px;height:70px;object-fit:contain"></td><td><p style="margin:0 0 8px;font-size:19px;letter-spacing:3px;font-weight:bold;color:#f9f8ef">DIETTEMPLE</p><p style="margin:0;color:#dfc477;font-size:10px;letter-spacing:2px">NUTRITION &amp; PERFORMANCE</p></td></tr></table><p style="margin:0 0 12px;color:#c6d2b8;font-size:11px;letter-spacing:2px">${admin ? 'NOUVELLE COMMANDE' : 'MERCI POUR VOTRE CONFIANCE'}</p><h1 style="margin:0;color:#f9f8ef;font-size:30px;line-height:38px;letter-spacing:-1px">${admin ? 'Une commande à préparer.' : 'Votre commande<br>est confirmée.'}</h1><p style="margin:18px 0 0;color:#dfc477;font-size:13px;line-height:20px">Référence ${e(order.reference)}</p></td></tr>
    ...
    <p style="margin:0 0 8px;font-size:10px;letter-spacing:2px;font-weight:bold;color:#65705b">VOS ESSENTIELS NUTRITION</p><h2 style="margin:0;font-size:21px;color:#20271b">Le détail de votre sélection</h2><table width="100%" cellspacing="0" cellpadding="0" aria-label="Articles commandés">${rows}</table>
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-top:18px">${summary('Sous-total', amount(order.subtotal))}${order.discount > 0 ? summary('Réduction', `−${amount(order.discount)}`) : ''}${summary('Livraison', order.deliveryFee ? amount(order.deliveryFee) : 'Offerte')}<tr><td style="padding:20px 12px;background:#182112;color:#f9f8ef;font-size:16px;font-weight:bold">Total</td><td align="right" style="padding:20px 12px;background:#182112;font-size:23px;font-weight:bold;color:#dfc477">${e(amount(order.totalPrice))}</td></tr></table>
